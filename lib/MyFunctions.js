Meteor.myFunctions = {
    ...
    createTagSuggestion : function (tagName){
      ...
      $('#suggestionsContainer').css('display', 'inline-block');
      if (!document.getElementById(tagName)) {
      // create div
        var parentNode = document.getElementById('suggestionsContainer');
    ...
    */
    ...
    findCityAC : function(inputedLetters) {
      ...
      Meteor.call('findCityACServerSide', function(err, response){
                if (err) {
                  console.log(err);
                } else {
